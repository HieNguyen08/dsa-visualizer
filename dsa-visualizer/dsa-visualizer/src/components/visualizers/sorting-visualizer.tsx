...
const SortingVisualizer = () => {
  ...
  const [highlightedIndices, setHighlightedIndices] = useState<{[key: number]: 'compare' | 'swap'}>({});
  ...
  const animateSort = (animations: Animation[]) => {
    ...
    let currentArrayState = [...array]; // Local copy to track changes
    ...
    const animate = () => {
      ...
      if (animation.type === 'compare') {
        ...
        // Perform the actual swap in our local state
  ...
  const getBarColor = (index: number) => {
    if (isSorted) return SORTED_COLOR;
    if (highlightedIndices[index]) {
      return highlightedIndices[index] === 'compare' ? COMPARE_COLOR : SWAP_COLOR;
    }
